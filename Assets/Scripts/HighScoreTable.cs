...
public class HighScoreTable : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            Debug.Log("HighScores not set");
            highScores = new HighScores();
            highScores.highScoreEntryList =
            ...
            PlayerPrefs
                .SetString("highScoreTable", JsonUtility.ToJson(highScores));
            PlayerPrefs.Save();
        }
        ...
        for (int i = 0; i < highScores.highScoreEntryList.Count; i++)
        {
            for (int j = 0; j < highScores.highScoreEntryList.Count; j++)
            {
                if (
                    ...
                    highScores.highScoreEntryList[j].score
                )
                {
                    //swap
